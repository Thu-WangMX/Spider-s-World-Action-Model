...
def create_fastwam_dino(
    ...
    from .models.wan22.fastwam_dino import FastWAM_DINO
    ...
    if isinstance(action_scheduler, DictConfig):
        action_scheduler = OmegaConf.to_container(action_scheduler, resolve=True)
    if action_scheduler is None:
        raise ValueError("`action_scheduler` is required for FastWAM_DINO.")
    if not isinstance(action_scheduler, dict):
        raise ValueError(f"`action_scheduler` must be dict-like, got {type(action_scheduler)}")
    required_action_scheduler_keys = {"train_shift", "infer_shift", "num_train_timesteps"}
    missing_keys = required_action_scheduler_keys - set(action_scheduler.keys())
...
def create_fastwam_vae_dino_mot(
    dino_config,
    vae_video_dit_config,
    dino_video_dit_config,
    action_dit_config=None,
    ...
    redirect_common_files: bool = True,
    action_dit_pretrained_path: str | None = None,
    vae_video_dit_pretrained_path: str | None = None,
    dino_video_dit_pretrained_path: str | None = None,
    skip_dit_load_from_pretrain: bool = False,
    video_scheduler=None,
    action_scheduler=None,
    loss=None,
    ...
    from .models.wan22.fastwam_vae_dino_mot import FastWAMVAEDinoMoT
    ...
    if isinstance(vae_video_dit_config, DictConfig):
        vae_video_dit_config = OmegaConf.to_container(vae_video_dit_config, resolve=True)
    if not isinstance(vae_video_dit_config, dict):
        raise ValueError(
            f"`vae_video_dit_config` must resolve to a dict, got {type(vae_video_dit_config)}"
        )

    if isinstance(dino_video_dit_config, DictConfig):
        dino_video_dit_config = OmegaConf.to_container(dino_video_dit_config, resolve=True)
    if not isinstance(dino_video_dit_config, dict):
        raise ValueError(
            f"`dino_video_dit_config` must resolve to a dict, got {type(dino_video_dit_config)}"
        )
    ...
    if isinstance(action_scheduler, DictConfig):
        action_scheduler = OmegaConf.to_container(action_scheduler, resolve=True)
    if action_scheduler is None:
        raise ValueError("`action_scheduler` is required for FastWAMVAEDinoMoT.")
    if not isinstance(action_scheduler, dict):
        raise ValueError(f"`action_scheduler` must be dict-like, got {type(action_scheduler)}")
    required_action_scheduler_keys = {"train_shift", "infer_shift", "num_train_timesteps"}
    missing_keys = required_action_scheduler_keys - set(action_scheduler.keys())
    ...
    if isinstance(semantic_history_config, DictConfig):
        semantic_history_config = OmegaConf.to_container(semantic_history_config, resolve=True)
    if semantic_history_config is None:
        semantic_history_config = {}
    if not isinstance(semantic_history_config, dict):
        raise ValueError(
            f"`semantic_history_config` must resolve to a dict, got {type(semantic_history_config)}"
        )

    return FastWAMVAEDinoMoT.from_config(
        dino_config=dino_config,
        vae_video_dit_config=vae_video_dit_config,
        dino_video_dit_config=dino_video_dit_config,
        action_dit_config=action_dit_config,
        device=device,
        torch_dtype=model_dtype,
        model_id=model_id,
        ...
        redirect_common_files=bool(redirect_common_files),
        action_dit_pretrained_path=action_dit_pretrained_path,
        vae_video_dit_pretrained_path=vae_video_dit_pretrained_path,
        dino_video_dit_pretrained_path=dino_video_dit_pretrained_path,
        skip_dit_load_from_pretrain=bool(skip_dit_load_from_pretrain),
        mot_checkpoint_mixed_attn=bool(mot_checkpoint_mixed_attn),
        video_train_shift=float(video_scheduler.get("train_shift", 5.0)),
        video_infer_shift=float(video_scheduler.get("infer_shift", 5.0)),
