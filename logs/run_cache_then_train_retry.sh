...
CACHE_DIR=./data/dino_latents_cache/libero_dino_s_2cam224_pool1x2_frame
...
run_cache() {
  ...
  ${PYTHON} -m torch.distributed.run \
    ...
    2>&1 | tee -a logs/precompute_dino_frame_latents_b${bs}_emb${emb}_$(date +%Y%m%d_%H%M%S).log
}
...
run_train() {
  ...
  bash scripts/train_zero1.sh 4 \
    ...
    wandb.name=libero_dino_s_smallvideo_framecache_lv0.05_la5_bs${bs}ga${ga} \
    2>&1 | tee -a logs/train_libero_dino_s_smallvideo_framecache_bs${bs}ga${ga}_$(date +%Y%m%d_%H%M%S).log
}
